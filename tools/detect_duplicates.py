...
def detect_duplicates(backlog_dir="backlog/active"):
    """
    Legion 10 Defense: The Clones (Duplicate Tickets)
    Finds tickets with dangerously similar titles.
    """
    ...
    # 2. Check Similarity (O(N^2) but N is small for active backlog)
    # Using SequenceMatcher ratio > 0.85
    ...
    for i in range(len(titles)):
        for j in range(i + 1, len(titles)):
            ...
            if similarity > 0.85:
                ...
                duplicates += 1
                checked.add(file2) # Mark as checked/duplicate
